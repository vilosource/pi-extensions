 ...
 * attribute dictionary. Standard `gen_ai.*` GenAI attributes plus the
 * harness-agnostic `agent.*` namespace consumed by the token-tracker
 * backend's `/v1/traces` ingest.
 *
 * Adds `agent.cost.estimation`: distinguishes subscription-billed
 * providers (which return zero cost) from metered ones, so the dashboard
...
export function turnAttributes(
	...
	return {
		...
		// agent.user.id is diagnostic only — the server uses the verified
		// access-token claims for the authoritative identity (ADR D8).
